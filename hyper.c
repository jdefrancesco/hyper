		...
		for(j=0;j<2;++j) {
			for(k=0;k<2;++k) {
				for(l=0;l<2;++l) {
					...
					kt = k ? 1 : -1;
					lt = l ? 1 : -1;
					hvertices[count] = newVertex4d((float)it,(float)jt,(float)kt,(float)lt);
					++count;
				}
			}
		}
...
void setHypercubeEdges(struct edge4d* hedges)
{
	...
	for(;currentVertex < 16; ++currentVertex) {
		if( !usedEdge(currentVertex,currentVertex^inv0,used) ) {
			hedges[edgeCount] = newEdge4d(&hypercubeVertices[currentVertex],&hypercubeVertices[currentVertex^inv0]);
			++edgeCount;
		}
		if( !usedEdge(currentVertex,currentVertex^inv1,used) ) {
			hedges[edgeCount] = newEdge4d(&hypercubeVertices[currentVertex],&hypercubeVertices[currentVertex^inv1]);
			++edgeCount;
		}
		if( !usedEdge(currentVertex,currentVertex^inv2,used) ) {
			hedges[edgeCount] = newEdge4d(&hypercubeVertices[currentVertex],&hypercubeVertices[currentVertex^inv2]);
			++edgeCount;
		}
		if( !usedEdge(currentVertex,currentVertex^inv3,used) ) {
			hedges[edgeCount] = newEdge4d(&hypercubeVertices[currentVertex],&hypercubeVertices[currentVertex^inv3]);
			++edgeCount;
		}
	}
...
void rotate4d(float angle, int rotateCode, float* x1, float* x2, float* y1, float* y2, float* z1, float* z2, float* t1, float* t2)
{
